...
def readFile(file,sheetname):
    ...
    if (delete_columns_submit or st.session_state.step_two_btn) and keepColumnsList:
        ...
        # FIELDS
        emailcol = col1.multiselect('Choose primary key email column to cleanse. Please make sure the column name contains the word "email" (required)', columns_list)
        ...
        if cleanse_email_submit or st.session_state.step_three_btn and emailcol:
            ...
            if (len(dataframe) > 100000):
                col2.download_button('Downlaod Scrubbed CSV', dataframe.to_csv(index=False),mime='text/csv')
